...
def compute_idfs(documents):
    ...
    for word in unique_words:
        ...
        id_dictionary[word] = math.log(doc_len/count)
...
def top_files(query, files, idfs, n):
    ...
    sortedscore = [k for k, v in sorted(scores.items(), key=lambda x: x[1], reverse=True)]
...
def top_sentences(query, sentences, idfs, n):
    ...
    sortedscore = [k for k, v in sorted(scores.items(), key=lambda x: (x[1][0], x[1][1]), reverse=True)]
